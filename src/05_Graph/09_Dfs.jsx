...
function Dfs({ onBack }) {
    ...
    const handleStep = () => {
        ...
        if (stackRef.current.length === 0 && visitedRef.current.length === 0) {
            setVisited([start]);
            setStack([start]);
            setCurrent(start);
            setOrder([start]);
            setStatus(`Starting DFS at ${labelFromIndex(start)}.`);
            return;
        }
        ...
        <div className="array-shell">
            <div className="array-card">
                <div className="page-header">
                    <div>
                        <div className="array-title">Depth First Search (DFS)</div>
                        <div className="array-meta">Explore as deep as possible before backtracking</div>
                    </div>
                ...
                <div className="info-grid">
                    <div className="info-card">
                        <h3>How DFS Works</h3>
                        <p>It follows one branch as far as possible using a stack.</p>
                    </div>
                    <div className="info-card">
                        <h3>Stack Order</h3>
                        <p>Last discovered node is explored next (LIFO).</p>
                    </div>
                </div>
                ...
                <div className="section-block">
                    ...
                    <div className="queue-list">
                        {stack.length === 0 ? (
                            <span className="list-empty">Stack is empty</span>
                        ) : (
                            stack
                                .slice()
                                .reverse()
                                .map((node) => (
                                    <span className="queue-chip" key={`${node}-stack`}>
                                        {labelFromIndex(node)}
                                    </span>
                                ))
                        )}
                ...
                <div className="section-block">
                    ...
                    <div className="graph-shell">
                        <svg className="graph-canvas" viewBox="0 0 520 360" role="img" aria-label="DFS visualization">
                            <defs>
                                <marker
                                    id="arrowhead-dfs"
                                    markerWidth="12"
                            ...
                            {graphLayout.edges.map((edge) => (
                                <g key={edge.id}>
                                    <line
                                        ...
                                        x2={edge.line.to.x}
                                        y2={edge.line.to.y}
                                        markerEnd={directed ? 'url(#arrowhead-dfs)' : undefined}
                                    />
                        ...
                        <div className="graph-legend">
                            ...
                            <span className="graph-legend-item">Green edges: DFS tree</span>
