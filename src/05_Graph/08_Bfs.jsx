...
function Bfs({ onBack }) {
	...
	const handleStep = () => {
		...
		if (queueRef.current.length === 0 && visitedRef.current.length === 0) {
			setVisited([start]);
			setQueue([start]);
			setCurrent(start);
			setOrder([start]);
			setStatus(`Starting BFS at ${labelFromIndex(start)}.`);
			return;
		}
	...
	const stepTraversal = () => {
		...
		if (queueNow.length === 0) return;
		const [node, ...rest] = queueNow;
		...
		adjList[node].forEach((edge) => {
			if (!visitedSet.has(edge.to)) {
				...
				if (nextParents[edge.to] === undefined) {
					nextParents[edge.to] = node;
				}
				rest.push(edge.to);
			}
		});
		const nextOrder = orderRef.current.includes(node) ? orderRef.current : [...orderRef.current, node];
		setCurrent(node);
		setVisited(Array.from(visitedSet));
		setQueue(rest);
		setParents(nextParents);
		setOrder(nextOrder);
		setStatus(`Exploring ${labelFromIndex(node)}.`);
	};
		...
		<div className="array-shell">
			<div className="array-card">
				<div className="page-header">
					<div>
						<div className="array-title">Breadth First Search (BFS)</div>
						<div className="array-meta">Explore neighbors level by level</div>
					</div>
				...
				<div className="info-grid">
					<div className="info-card">
						<h3>How BFS Works</h3>
						<p>It visits nodes in layers, using a queue to expand the frontier.</p>
					</div>
					<div className="info-card">
						<h3>Queue Order</h3>
						<p>Nodes are processed in the order they are discovered.</p>
					</div>
				</div>
				...
				<div className="section-block">
					...
					<div className="queue-list">
						{queue.length === 0 ? (
							...
							queue.map((node) => (
								<span className="queue-chip" key={`${node}-queue`}>
									{labelFromIndex(node)}
								</span>
							))
						)}
				...
				<div className="section-block">
					...
					<div className="edge-list">
						{order.length === 0 ? (
							<span className="list-empty">No nodes visited yet.</span>
						) : (
							order.map((node) => (
				...
				<div className="section-block">
					...
					<div className="graph-shell">
						<svg className="graph-canvas" viewBox="0 0 520 360" role="img" aria-label="BFS visualization">
							<defs>
								<marker
									id="arrowhead-bfs"
									markerWidth="12"
							...
							{graphLayout.edges.map((edge) => (
								<g key={edge.id}>
									<line
										...
										x2={edge.line.to.x}
										y2={edge.line.to.y}
										markerEnd={directed ? 'url(#arrowhead-bfs)' : undefined}
									/>
							...
							{graphLayout.nodes.map((node) => {
								...
								const nodeClass = `graph-node${
									isCurrent ? ' graph-node-current' : isVisited ? ' graph-node-visited' : ''
								}`;
								return (
									<g key={node.id}>
						...
						<div className="graph-legend">
							...
							<span className="graph-legend-item">Green edges: BFS tree</span>
							<span className="graph-legend-item">Highlight shows visited/current</span>
						</div>
					</div>
				</div>
			</div>
